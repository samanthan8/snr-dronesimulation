...
results_dir_pso = "pso_simulation_results_csv"  # Update with your results directory

all_results_pso = []
for filename in os.listdir(results_dir_pso):
    if filename.endswith(".csv"):
        df = pd.read_csv(os.path.join(results_dir_pso, filename))
        all_results_pso.append(df)

results_df_pso = pd.concat(all_results_pso)
...
for num_drones, group_data in results_df_pso.groupby("num_drones"):
...
plt.title("PSO Performance vs. Number of Drones")

plt.subplot(1, 2, 2)
plt.plot(success_rate.index, success_rate.values, marker='o', color='green')
plt.xlabel("Number of Drones")
plt.ylabel("Success Rate")
plt.title("PSO Success Rate vs. Number of Drones")
