...
results_dir_aco = "aco_simulation_results_csv"  # Update with your results directory

all_results_aco = []
for filename in os.listdir(results_dir_aco):
    if filename.endswith(".csv"):
        df = pd.read_csv(os.path.join(results_dir_aco, filename))
        all_results_aco.append(df)

results_df_aco = pd.concat(all_results_aco)

# Calculate average iterations and success rate for each number of drones
avg_iterations = results_df_aco.groupby("num_drones")["iterations"].mean()
success_rate = results_df_aco.groupby("num_drones")["target_found"].mean()
...
plt.subplot(1, 2, 1)
plt.scatter(results_df_aco["num_drones"], results_df_aco["iterations"], alpha=0.5, label="Individual Runs")
...
plt.title("ACO Performance vs. Number of Drones")

plt.subplot(1, 2, 2)
plt.plot(success_rate.index, success_rate.values, marker='o', color='green')
plt.xlabel("Number of Drones")
plt.ylabel("Success Rate")
plt.title("ACO Success Rate vs. Number of Drones")
